...
export function checkReachability(urlString) {
    return new Promise((resolve) => {
        ...
        const request = (method) => {
            ...
            req.on("error", (error) => finish({ online: false, method, error: error.message }));
            req.end();
        };
        request("HEAD");
    });
}
async function checkReachabilityReliably(url) {
    const firstResult = await checkReachability(url);
    return firstResult.online ? firstResult : checkReachability(url);
}
async function checkReachabilityCached(url) {
...
export async function checkReachabilities(urls) {
