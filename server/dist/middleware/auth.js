...
import { completeOidcAuthorization, createOidcAuthorizationUrl, getAdminOidcConfig, getOidcStatus, updateAdminOidcConfig, } from "../services/oidcService.js";
...
async function writeAdminPassword(password) {
...
function createAdminSession(request, response, authMethod, identity) {
...
authRouter.get("/auth/oidc/callback", async (request, response) => {
    try {
        ...
        if (previousSession)
            sessions.delete(previousSession.id);
...
authRouter.post("/auth/logout", requireAdmin, (_request, response) => {
    const authenticated = response.locals.adminSession;
    if (authenticated)
