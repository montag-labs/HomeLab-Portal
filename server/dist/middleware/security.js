...
function removeExpiredStatusRequests(now = Date.now()) {
    for (const [address, entry] of statusRequests) {
        if (entry.resetAt <= now)
            statusRequests.delete(address);
    }
}
function setBoundedStatusRequest(address, entry) {
    statusRequests.delete(address);
...
export function securityHeaders(_request, response, next) {
    ...
    response.setHeader("Content-Security-Policy", "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: http: https:; connect-src 'self'; frame-src http: https:");
    next();
}
export function limitStatusRequests(request, response, next) {
    ...
    if (entry.count > MAX_STATUS_REQUESTS)
        return response.status(429).json({ error: "Too many status requests" });
    next();
}
