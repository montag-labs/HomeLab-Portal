...
export async function readUpdateToken() {
    try {
...
export async function getPendingUpdateToken() {
...
export async function acknowledgeUpdateToken(candidate) {
    const token = await readUpdateToken();
    if (!token)
        return false;
...
export async function resetUpdateToken(token) {
    await mkdir(TOKEN_DIR, { recursive: true, mode: 0o700 });
