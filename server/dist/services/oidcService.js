...
function withOpenIdScope(value) {
...
function environmentConfig() {
    ...
    if (!environmentKeys.some((key) => Boolean(process.env[key]?.trim())))
        return undefined;
    ...
    const clientAuthMethod = clientAuthMethodSchema.safeParse(configuredMethod).success
        ? configuredMethod
...
function isSecureApplicationUrl(value) {
    try {
...
function validateEnabledConfig(config) {
    if (!config.enabled)
        return;
...
function getClientAuthentication(config) {
...
async function getConfiguration(config) {
    ...
    if (cachedConfiguration?.fingerprint !== configFingerprint) {
        const promise = oidc.discovery(new URL(config.issuerUrl), config.clientId, config.clientSecret ? { client_secret: config.clientSecret } : undefined, getClientAuthentication(config)).catch((error) => {
            if (cachedConfiguration?.promise === promise)
...
function readClaim(claims, claimPath) {
    return claimPath.split(".").reduce((current, key) => {
        if (!current || typeof current !== "object" || Array.isArray(current))
            return undefined;
        return current[key];
    }, claims);
}
function hasAllowedGroup(claims, config) {
...
export async function updateAdminOidcConfig(input) {
    if (environmentConfig())
        throw new Error("OIDC configuration is managed by environment variables");
...
async function markStoredConfigVerified(expectedFingerprint) {
    if (environmentConfig())
        return;
    const current = await readStoredConfig();
...
export async function completeOidcAuthorization(callbackUrl) {
    ...
    if (!claims || typeof claims.sub !== "string")
        throw new Error("OIDC ID token has no subject");
    if (!hasAllowedGroup(claims, config))
        throw new Error("OIDC user is not in an allowed admin group");
